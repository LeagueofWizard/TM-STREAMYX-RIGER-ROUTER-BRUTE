...
namespace TM_router
{
    class Program
    {
        ...
        async static void Foo()
        {
            ...
            string success = target + "/rpSys.html";
            ...
            while (isSuccess != true)
            {
                ...
                var response = await client.PostAsync(target + "/Forms/TM2Auth_1", content);
                Uri responseString = response.Headers.Location;
                ...
                {
                    Console.WriteLine("Attempt {0} failed. TEST: {1}, RESULT: {2} {3}", count, values["LoginPasswordValue"], responseString.AbsoluteUri, success);
                }
                //175.144.46.41
                count++;
            }
